...
class WxPayApi
{
	...
	public static function unifiedOrder($config, $inputObj, $timeOut = 6)
	{
		...
		}else if(!$inputObj->IsTotal_feeSet() || $inputObj->GetTotal_fee() <= 0) {
            throw new WxPayException("缺少统一支付接口必填参数total_fee或total_fee小于等于0！");
        }
		...
		\think\facade\Log::record($response);
    ...
    /**
     * 企业付款到零钱接口
     * @param $config WxAppPayConfig
     * @param $inputObj WxPayTransfers
     * @param int $timeOut
     * @return array|bool
     * @throws WxPayException
     */
    public static function transfers($config, $inputObj, $timeOut = 6)
    {
        ...
        if(!$inputObj->IsPartner_trade_noSet()) {
            throw new WxPayException("企业付款到零钱接口中，缺少必填参数partner_trade_no！");
            ...
            throw new WxPayException("退款申请接口中，缺少必填参数spbill_create_ip！");
        }
        $inputObj->SetMch_appid($config->GetAppId());//公众账号ID
        $inputObj->SetMchid($config->GetMerchantId());//商户号
        $inputObj->SetNonce_str(self::getNonceStr());//随机字符串
        $inputObj->SetCheck_name("NO_CHECK");//公众账号ID
